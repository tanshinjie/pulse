...
if pulse test-notification; then
    ...
    echo ""
    echo "📚 Quick Start:"
    echo "   pulse start              # Start tracking"
    echo "   pulse log \"message\"       # Log an activity"
    echo "   pulse status             # Check recent activities"
    echo "   pulse report             # View time summary"
    echo "   pulse stop               # Stop tracking"
    echo ""
    echo "📖 For more help: pulse --help"
    echo ""
    
    # Platform-specific tips
    if [[ "$OSTYPE" == "darwin"* ]]; then
        echo "🍎 macOS: Enable notifications in System Preferences → Notifications & Focus"
        echo "   Test: pulse test-notification"
        echo ""
    elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
        echo "🐧 Linux: Install libnotify-bin if notifications don't work"
        echo "   Test: pulse test-notification"
        echo ""
    fi
